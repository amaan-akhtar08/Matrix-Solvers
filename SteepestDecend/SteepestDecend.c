...
void steepestDecend(long n, double *A, double*b, double *x, long max_iterations, double tolerance, long *converging_iteration){
    ...
    multiply_Ax(n,A,x,Ax);   //Ax
    subtract_xy(n,b,Ax,r);   //r = b - Ax
    
    for(long k=0;k<max_iterations;k++){
        multiply_Ax(n,A,r,p);   //p = Ar

        double r_dot_r = multiply_xy(n, r, r);
        ...
        alpha = r_dot_r / multiply_xy(n, r, p);
...
int main(){
    ...
    Fvec = readFile_To_vec("D:/matrix-solvers-2/SteepestDecend/input_mat/Fvec.txt", &n);
    Kmat = readFile_To_vec("D:/matrix-solvers-2/SteepestDecend/input_mat/Kmat.txt", &size);
    ...
    steepestDecend(n,Kmat,Fvec,Xvec,max_iterations,tolerance,&converging_iteration);
    ...
    elapsed = seconds + useconds / 1000000.0;
    printf("#Steepest-Decend Solver Time taken: %f seconds\n\n", elapsed);
    
    writeToFile("D:/matrix-solvers-2/output/SteepestDecend-Sol.txt",Xvec,converging_iteration,n);
...
void execute_py(bool* flag){
    ...
    int result = system("python3 D:/matrix-solvers-2/SteepestDecend/ConvRate.py");
