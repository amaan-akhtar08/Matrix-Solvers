...
for(i=0;i<nnode;i++)
{
  if(coord[mnode*i+0]==0&&coord[mnode*i+1]!=0&&coord[mnode*i+1]!=1)  //left
  {
    bcnode[k]=i+1;
    bcval[k]=0;
    k++;
  }
  if(coord[mnode*i+0]==1&&coord[mnode*i+1]!=0&&coord[mnode*i+1]!=1)     //right
  {
    bcnode[k]=i+1;
    bcval[k]=0;
    k++;
  }
  if(coord[mnode*i+1]==0)     //bottom
  {
    bcnode[k]=i+1;
    bcval[k]=0;
    k++;
  }
  if(coord[mnode*i+1]==1)       //top
  {
    bcnode[k]=i+1;
    bcval[k]=1;
    k++;
